"""NeuralUQ for Darcy problem using PA-BNN-FP"""
...
if __name__ == "__main__":
    ...
    ################################ Build processes #################################
    # build process_f, which is a Bayesian neural network
    layers = [2, 50, 50, 1]
    prior_shared = neuq_vars.fnn.Samplable(layers=layers, mean=0, sigma=1)
    process_f = neuq.process.Process(
    surrogate=neuq.surrogates.FNN(layers=layers), prior=prior_shared,
    )
